...
- **Decision branches:**
  ...
  - B. **1.2a is `[GATE]`** — process topology × DB ADR (one machine vs one-per-persona; SQLite-on-volume vs Postgres). The full constraint set is already researched and written into `BUILD_PLAN.md` itself (Fly volume single-attach, LiteFS disqualified, cost trade-off) — read it there before the Alex conversation, don't re-research from scratch.
