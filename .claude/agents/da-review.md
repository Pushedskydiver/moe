...
5. Consult `docs/REVIEW-PATTERNS.md` for recurring issue classes applicable to the diff — including the five patterns pre-seeded from `docs/VISION.md` §12, several of which describe functionality that doesn't exist yet (a persona, the core-hours guard) and so can't have fired on any diff before that functionality lands.
