...
Before walking the architectural checklists, extract every verifiable claim the diff makes about the codebase and verify each one against ground truth. A claim is any prose or code assertion that can be proven false by reading another file. Seven buckets (straddles are fine — extract under every bucket that fits):
...
- **Quoted/attributed claim** — any phrase in quote-marks attributed to another doc (`docs/VISION.md`, an ADR, an external API's own docs). Grep the exact substring against the cited source directly — don't trust that it's accurate because it "reads like" the source's style, and don't trust a re-read of the finding that first introduced the quote over a direct check. Two confirmed failure shapes: a phrase presented as a direct citation that never appears in the source at all (leftover placeholder text from an earlier draft, later dressed in quote-marks and misattributed); and a paraphrase presented in quote-marks as if verbatim, dropping or altering a few words from the real source text. A misquote caught and fixed in one location often recurs in sibling locations (a TSDoc, a doc table row, a chunk writeup all citing the same fabricated phrase) — grep the whole repo for the exact string, not just the flagged file, and re-check any later addition to the same PR that touches the same claim.
...
- [ ] Bug fixes include a reproduction test that **failed** before the fix, not just a test added after — the Prove-It Pattern, `docs/TESTING.md` §Bug fixes — the Prove-It Pattern
