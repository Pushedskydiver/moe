...
- **Function verb vocabulary.** `make*` (factory returning a closure/adapter), `create*` (single-call factory returning a fully-initialised object), `build*` (multi-step structured-value construction), `resolve*` (derivation with a fallback/lookup chain), `parse*` (unstructured → typed), `detect*` (environment/runtime probe), `ensure*` (idempotent side-effect), `fetch*` (external I/O), `validate*` (I/O-bound precondition/credential check, returns a Result — no data on success, unlike `fetch*`; first used by `packages/github/src/validate-github-credentials.ts`'s boot-time guard, BUILD_PLAN 4.1), `upsert*` (insert-or-update-in-place against a natural key, via `onConflict().doUpdateSet()` — `record*`'s own existing repository functions, `recordSweepCompleted`/`recordUsage`, are upserts under the hood too, so the naming split isn't about the SQL shape: `record*` names a function that tracks this app's own bookkeeping — a sweep timestamp, an accumulated usage total — while `upsert*` names one whose whole point is mirroring an external entity's current row; first used by `packages/core/src/intake/github-issue-triage-repository.ts`'s `upsertGithubIssueTriageEntry`, BUILD_PLAN 4.2, which refreshes a GitHub issue's title/state to whatever GitHub reports on each poll), `find*` (search a pre-loaded collection), `evaluate*` (a decision derived from given inputs — no _direct_ raw I/O of its own, though it may call an already-injected dependency (a cache, a client) that does; no fallback/lookup chain, just an outcome; distinct from `resolve*`'s own lookup-with-fallback shape), `is*/has*/can*/should*` (boolean predicate), `wire*` (DI composition), `provision*` (multi-step creation of a real external resource via its own API — validate-then-create, returning the same discriminated-union Result shape as the rest of this list, but — unlike `validate*` — carrying real captured data (credentials, ids) on success; first used by `packages/slack/src/provision-persona-slack-app.ts`'s `provisionPersonaSlackApp`, BUILD_PLAN 5.1). Dropped: `compute*/calculate*` (leaks mechanism — use noun-phrase form), `attempt*/try*` (redundant when the return type already encodes failure), generic `get*/set*`.
...
- **Every new SDK client wires BOTH secret-redaction mechanisms from the start, not as a follow-up fix.** Key-based redaction (`apps/server/src/redact-secrets.ts`, matched by field name against `apps/server/src/main.ts`'s `SECRET_KEYS` list) and value-based redaction (a per-client `*SdkLoggerAdapter`, e.g. `packages/slack/src/create-sdk-logger-adapter.ts`, `packages/agents/src/create-anthropic-sdk-logger-adapter.ts`, `packages/github/src/create-github-sdk-logger-adapter.ts` — scrubs a known secret substring wherever it appears in free-text SDK log output, since most SDK loggers take positional args with no field names to match) are separate, independent implementations per client library — building only the "obvious" one passes typecheck and looks complete, but silently leaves the other mechanism un-applied. Confirmed live twice on two different SDKs (Slack, then Anthropic) before this rule was written down: a new integration site is exactly as exposed to this gap as an old one, being new is not exemption. Before adding a new secret-handling client, grep the repo for `SECRET_KEYS` and `SdkLoggerAdapter` to enumerate every current mirror location, wire both mechanisms together, and add a same-PR test asserting the new secret value is actually redacted at each one.
...
- **Prefer the Anthropic SDK's own `zodOutputFormat()` + `.messages.parse()` over raw `.messages.create()` + manual `JSON.parse()`** for any call needing structured output — the response is validated against the same Zod schema the caller's own return type is built from, one schema serving both. `.messages.create()` accepts `output_config.format` too but does no automatic parsing (per the SDK's own docs) — don't reach for it here. First real implementation: `packages/agents/src/classify-message-confidence.ts` (BUILD_PLAN chunk 3.3).
...
**Never name a `logger.error`/`.warn`/`.info` fields-object key `message`, `level`, or `timestamp`.** `apps/server/src/logger.ts`'s own `writeLine` deliberately lets the log line's own metadata win over a same-named caller field — a real, confirmed bug: 44 call sites (41 in `apps/server`, 3 in `packages/slack`'s `socket-mode-listener.ts` — any consumer wired to the real `AppLogger` implementation, not just `apps/server` itself) once used `message: someErrorDetail` for their own error-detail field, silently discarding it on every log line (BUILD_PLAN 4.2, caught via a live run — a repo-wide grep during DA review found the `packages/slack` sites a first pass had missed). Use `errorMessage` instead.
