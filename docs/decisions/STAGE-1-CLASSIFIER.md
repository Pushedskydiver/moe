...
- **Prefill is dead on every current-generation model moe would plausibly use.** Assistant-turn prefill (the classic pairing with log-probs-style techniques) explicitly returns a 400 on Opus 4.6/4.7/4.8 and Sonnet 4.6/5; on Fable 5 it's documented as "not supported" without the reference stating the literal HTTP status. Either way, prefill isn't usable on any of them.
...
- Claude API structured outputs (`output_config.format`), verified live against the `claude-api` skill's current reference — no log-probs anywhere in the Messages API; assistant-turn prefill returns 400 on Opus 4.6/4.7/4.8 and Sonnet 4.6/5, and is unsupported (status not stated) on Fable 5.
