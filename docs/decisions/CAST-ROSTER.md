...
## Status update (2026-07-24, BUILD_PLAN chunk 5.0)

**The 8th role, Designer (Maya), is activated — roster is now 8.** Not decided on this doc's own scope-matching trigger: a fresh check found chief-clancy had, in the intervening 9 days, actually grown a real UI codebase (`packages/design`, "Phase F" — a React 19 + Vite canvas app with real CSS/accessibility conventions, chief-clancy's own top-priority workstream). That looked at first like the named re-evaluation trigger firing — but Alex corrected the framing directly: it isn't the deciding factor. `packages/design` is a dev-tool design-review canvas within chief-clancy's own source, not an end-user product surface, and it's currently mid-rework/unstable (confirmed via chief-clancy's own PROGRESS.md — 8 previously-"shipped" slices being redone; per `packages/design/README.md`'s own warning, the browser canvas UI itself is still a later, not-yet-landed slice).

The actual driver: Alex's explicit steer to activate proactively, ahead of a second, real end-user-UI project (his own Colour-Contrast-Checker repo) he plans to bring into moe's scope once the single-project loop is proven on chief-clancy — rather than defer Designer until that need materializes, per this doc's original "Deferred / explicitly rejected" framing. Alex asked for a default-to-yes absent strong, real evidence otherwise. A fresh check for such evidence (arXiv:2602.01011/2604.02460/2512.17898 follow-ups, competing "AI coworker" product cast-composition disclosures, new Designer-role-timing research) found none published between 2026-07-15 and 2026-07-24 — so the original "no reliable evidence either way" conclusion stands unchanged, and the default-yes holds.

Also settled in the same conversation: chunk 5.3 (persona prompts) adopts a per-persona deep-research + adversarial-"grill" methodology — extending the existing two-phase `spec-grill` discipline (`docs/DEVELOPMENT.md`) from specs to persona prompts. Designer's own research pass (surveying Impeccable, Claude Design, and the broader AI-design-tool landscape, aimed at avoiding "AI slop") is banked ahead of its 5.3 sub-chunk at `.claude/research/designer-persona-landscape/landscape-survey.md`.

Updated: `docs/VISION.md` §4.1, `docs/PERSONAS.md` roster table (both do-not-touch, drafted with Alex directly), `BUILD_PLAN.md` chunks 5.0/5.3 and the Stage 5 header/exit criterion (N=7 → N=8).
