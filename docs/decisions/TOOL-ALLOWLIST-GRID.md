...
**Current state, audited directly against the codebase before drafting:** no MCP server exists anywhere in the repo. Exactly one tool was ever exposed to any persona's own LLM calls at this chunk's own drafting — `STATUS_CLAIM_TOOL`/`report_status` (`packages/agents/src/status-claim-tool.ts`). **Updated at BUILD_PLAN 6.1f:** a second tool, `REACT_TOOL`/`react` (`packages/agents/src/react-tool.ts`), now exists, built and unit-tested — but the two call sites this doc originally named have since diverged: production's live `tools` array (`generate-and-post-reply.ts`) still passes `STATUS_CLAIM_TOOL` only (`REACT_TOOL` is deliberately not wired in there yet, pending persona prompt-grounding work), while the manual, real-API replay-recording script (`docs/decisions/PERSONA-REPLAY-HARNESS.md` decision 1) now passes both. So "exactly one tool is exposed" no longer holds uniformly across every call site — it remains true of the one that matters for real Slack traffic. (Two distinct senses of "live" appear in this document, and shouldn't be conflated: the replay script is "live" as in hitting the real Anthropic API rather than a mock, regardless of whether a given tool is "live" as in reachable by real production Slack traffic — the `react` grid row below tracks the latter sense only.) Every other integration (Slack `postMessage`/`addReaction`/`fetchSlackStatus`, GitHub `listOpenIssues`/`getGithubIssueState`/`createGithubIssue`, every DB table) is direct code the model never invokes — the persona's own prose triggers it, not a tool call. Riley's Agent SDK sandbox (chunk 6.2) has no code and no dependency installed yet. This chunk was therefore prospective at its own drafting — it specified the grid for tools that didn't exist yet then (Theo's web-search/citation-fetch, Maya's render/screenshot tool, Dom's PR-diff-fetch/review-post, and a generic cross-persona "react" tool flagged at chunk 6.1d — since built, mechanism-only, at chunk 6.1f), not a retrofit onto something already too permissive.
...
| `react`                       | All 8     | Built, not live | Direct — Slack `reactions.add` (`packages/slack/src/add-reaction.ts`), exposed as a model-invokable tool, `packages/agents/src/react-tool.ts` — not yet included in `generate-and-post-reply.ts`'s own live `tools` array (BUILD_PLAN 6.1f)                                                                                                                                                               |
