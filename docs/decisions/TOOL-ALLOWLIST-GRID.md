...
**Current state, audited directly against the codebase before drafting:** no MCP server exists anywhere in the repo. Exactly one tool was ever exposed to any persona's own LLM calls at this chunk's own drafting — `STATUS_CLAIM_TOOL`/`report_status` (`packages/agents/src/status-claim-tool.ts`). **Updated at BUILD_PLAN 6.1f:** a second tool, `REACT_TOOL`/`react` (`packages/agents/src/react-tool.ts`), now exists, built and unit-tested — but the two call sites this doc originally named had diverged: production's live `tools` array (`generate-and-post-reply.ts`) passed `STATUS_CLAIM_TOOL` only (`REACT_TOOL` deliberately not wired in yet, pending persona prompt-grounding work), while the manual, real-API replay-recording script (`docs/decisions/PERSONA-REPLAY-HARNESS.md` decision 1) already passed both. **Updated at BUILD_PLAN 6.1g:** that divergence is resolved — every persona's own `prompt.md` now grounds `react` (do-not-touch, drafted with Alex), and production's live `tools` array carries both `STATUS_CLAIM_TOOL` and `REACT_TOOL`, matching the replay script exactly. The "two distinct senses of live" caveat this section previously carried (real-API-reachable vs. production-traffic-reachable) no longer needs disambiguating for `react` specifically, since both senses now agree. Every other integration (Slack `postMessage`/`addReaction`/`fetchSlackStatus`, GitHub `listOpenIssues`/`getGithubIssueState`/`createGithubIssue`, every DB table) is direct code the model never invokes — the persona's own prose triggers it, not a tool call. Riley's Agent SDK sandbox (chunk 6.2) has no code and no dependency installed yet. This chunk was therefore prospective at its own drafting — it specified the grid for tools that didn't exist yet then (Theo's web-search/citation-fetch, Maya's render/screenshot tool, Dom's PR-diff-fetch/review-post, and a generic cross-persona "react" tool flagged at chunk 6.1d — built mechanism-only at chunk 6.1f, live in production traffic as of chunk 6.1g), not a retrofit onto something already too permissive.
...
| `react`                       | All 8     | Live      | Direct — Slack `reactions.add` (`packages/slack/src/add-reaction.ts`), exposed as a model-invokable tool, `packages/agents/src/react-tool.ts` — included in `generate-and-post-reply.ts`'s own live `tools` array, grounded in every persona's own `prompt.md` (BUILD_PLAN 6.1g)                                                                                                                          |
...
- Git allowed within the worktree, but local-only (`add`/`commit` to the task branch) — no `push`. Pushing and opening the PR is chunk 6.3a's own separate, credentialed step ("Branch/push from the 6.2 worktree... the GitHub write path"), running outside the sandboxed session's own hermetic Bash tool, not something the no-network sandbox does itself. Never a direct push to a protected branch, from either step. Code lands only via PR + CI (VISION §11, unchanged).
- No secrets access — `ANTHROPIC_API_KEY`, the DB connection string, and the GitHub App private key are not visible inside the sandboxed session's own environment. (The credential 6.3a's own push step needs is that step's problem to solve, not this sandbox's.)
