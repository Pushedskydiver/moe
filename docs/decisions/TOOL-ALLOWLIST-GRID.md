...
**Current state, audited directly against the codebase before drafting:** no MCP server exists anywhere in the repo. Exactly one tool is ever exposed to any persona's own LLM calls today — `STATUS_CLAIM_TOOL`/`report_status` (`packages/agents/src/status-claim-tool.ts`), passed at two call sites (`generate-and-post-reply.ts`, and the manual, real-API replay-recording script, `docs/decisions/PERSONA-REPLAY-HARNESS.md` decision 1 — live, not offline; only the test that replays its output is network-free). Every other integration (Slack `postMessage`/`addReaction`/`fetchSlackStatus`, GitHub `listOpenIssues`/`getGithubIssueState`/`createGithubIssue`, all ten DB tables) is direct code the model never invokes — the persona's own prose triggers it, not a tool call. Riley's Agent SDK sandbox (chunk 6.2) has no code and no dependency installed yet. This chunk is therefore prospective — it specifies the grid for tools that don't exist yet (Theo's web-search/citation-fetch, Maya's render/screenshot tool, Dom's PR-diff-fetch/review-post, a generic cross-persona "react" tool flagged at chunk 6.1d), not a retrofit onto something already too permissive.

**Updated at BUILD_PLAN 6.1f:** `REACT_TOOL` now exists (`packages/agents/src/react-tool.ts`), built and unit-tested, but the two call sites named above have diverged — production's live `tools` array (`generate-and-post-reply.ts`) stays exactly `[STATUS_CLAIM_TOOL]`; the replay harness's own array (`record-persona-replay.ts`, manual, real-API, no production traffic) is `[STATUS_CLAIM_TOOL, REACT_TOOL]` as of this chunk. So "exactly one tool is ever exposed" is no longer true of every call site uniformly — it is still true of the one that matters for real Slack traffic. The two nearby uses of "live" in this document track different axes and shouldn't be conflated: the replay script is "live" in the sense of hitting the real Anthropic API (as opposed to offline/mocked), while the grid row below's "Built, not live" tracks whether a tool is reachable by real production traffic at all. `REACT_TOOL` is live by the first sense (real-API-reachable, via the replay harness) but not by the second (no live persona can trigger it from a real Slack message yet).
