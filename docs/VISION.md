...
3. **Stage 2 — confidence-banded routing, never binary:**
   - **High confidence** → auto-draft the ticket, post it back where it can be seen — into the thread on an ambient message, top-level on a DM (see below) — as a visible, reversible draft using the existing reaction-gate pattern (📦/🔁/✅). A wrong high-confidence guess costs one ignored/corrected draft, never a silent action.
   ...
   **On a DM, the same three bands route differently in one respect** (settled with Alex at BUILD_PLAN chunk 3.7, when DMs first entered this cascade). High and Mid are unchanged in _what_ they produce — a draft or a confirming question — but not in _where_ it lands: a DM-triggered post goes **top-level in the DM**, not threaded on the source message the way an ambient one is. Threading it would leave the DM showing only the user's own message plus a "1 reply" affordance, hiding both the draft and its reaction legend behind a click, on the one surface that is never silent and where the reaction gate only works if it is seen. **Low is not**: a DM already gets a conversational reply, so "do nothing visible" doesn't apply, and the reply _is_ the non-silent outcome this bullet's own "nothing is silently eaten" principle asks for. No review-queue row is written for a Low-band DM — logging every "thanks, that's great" would bury the 3.5 sweep digest in chatter and make the backstop less useful, not more. The queue's purpose is to catch what nobody answered; a DM that got a real answer isn't that.

   **The governing invariant on a DM: the cascade may only ever _add_ to the response, never remove it.** The ambient path is silent by construction — every guard and every failure returns without posting, because a message nobody addressed expects no answer. The DM path is never silent. So High/Mid replace the chat reply only when everything succeeds; any guard block, classifier failure or failed post falls through to the ordinary reply. Concretely, that also means the §6.4/§14 operating-rhythm guard and §9's situational-appropriateness gate do **not** gate a DM-triggered draft — both exist to gate _unprompted_ posting, and a draft composed because someone sent a DM is as reactive as the reply it replaces. Spend is not rest: the per-persona cost cap still applies.
...
**Model-tier note (added 2026-07-15, updated 2026-07-24 for Claude Opus 5's release — informational, not a pin):** Claude Sonnet 5 (`claude-sonnet-5`, released 2026-06-30) is the concrete model "Sonnet-by-default" currently resolves to — introductory pricing of $2/$10 per MTok runs through 2026-08-31, then $3/$15 standard ([Anthropic — Claude models overview](https://platform.claude.com/docs/en/about-claude/models/overview), [pricing](https://platform.claude.com/docs/en/about-claude/pricing)). This doesn't change the policy above — every current call site still hardcodes one of exactly two models (`claude-sonnet-5` in `generate-reply.ts`/`compose-ticket-draft.ts`, `claude-haiku-4-5` in the Stage-1 classifier gate's `classify-message-confidence.ts`/`evaluate-situational-appropriateness.ts`), not a persona-specific split, and "per-persona model tuning as real data comes in" still governs whether/when that changes. Two persona-specific flags worth carrying into the chunk that actually wires a model in, re-verified directly against both system cards' own PDFs as of Opus 5's 2026-07-24 release (not a secondary-source summary): Sonnet 5 narrows the agentic-coding gap to Opus 4.8 generation-over-generation (SWE-bench Pro 63.2% for Sonnet 5 vs. 69.2% for Opus 4.8, a 6.0-point gap, down from Sonnet 4.6's own 11.1-point deficit) — but that's the wrong comparison to plan against now that Opus 5 (79.2%) supersedes Opus 4.8 as the actual working assumption: the cross-tier gap _widens_ to 16.0 points once the Opus reference itself moves (Anthropic's own [Claude Sonnet 5 System Card](https://www-cdn.anthropic.com/480e0bb54327b9622282e9c39a83a4f490ed377e/Claude%20Sonnet%205%20System%20Card.pdf), Table 8.1.A, for the 63.2% figure; [Claude Opus 5 System Card](https://www-cdn.anthropic.com/c5fbac3f0b1280a933ebd26d3cb8bb9f5bdeaf48/Claude%20Opus%205%20System%20Card.pdf), Table 8.1.A, for 79.2%/69.2% — this also corrects the original 2026-07-15 citation, which attributed the 69.2% Opus 4.8 figure to the Sonnet 5 System Card; that document's own SWE-bench Pro table never includes an Opus-tier model, only Sonnet 4.6 and competitor models). Sonnet 5 also has a documented cybersecurity-training gap versus Opus 4.8 specifically, per that same Sonnet 5 System Card's own §3.1.1 ("cyber capabilities of Sonnet 5 are... not as strong as those of Opus 4.8") — no primary source yet directly tests Sonnet 5 against Opus 5 on this axis, so this half of the claim stays anchored to the pairing it was actually measured against rather than extrapolated onto Opus 5. Net: treat Opus-tier (now Opus 5) as the working assumption for Riley's heavyweight chunk 6.2 coding work until moe has its own eval data, rather than assuming Sonnet 5 is "good enough now" by default — if anything a stronger case than at this note's original 2026-07-15 writing. Independently, [CodeRabbit's study of real pull requests with pre-identified bugs](https://www.coderabbit.ai/blog/claude-sonnet-5-review) found Sonnet 5's bug-catch recall (50–51%) is _worse_ than Sonnet 4.6's (63%) despite better precision (29%→38–40%) — relevant to Dom's review work specifically, worth validating with moe's own data before assuming the precision gain implies a recall gain, and unaffected by Opus 5's release since it never compared against an Opus-tier model.
...
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Sarah, Nia   | 1 (permission), 2 (preamble), 6 (priority), 5 (calibrated)                                                                                                                                                                                                              |
...
| Marcus       | 1 (permission), 2 (preamble), 6 (priority), 5 (calibrated), 4 (form) — plan/architecture critique is structurally close to Priya's and Maya's citation-required domains; a rejected alternative or a flagged risk needs a named, specific reason, not just a preference |
