...
**Open question, definitional not tunable, needed before this tier model is buildable for the common case:** "track record" is directory-scoped, but the document doesn't yet define what happens for a diff spanning multiple directories within one package (short of the package-boundary rule that routes it to Tier 2), a directory rename/move, or a brand-new directory. These aren't edge cases — they're common in ordinary software work — and should close in the same BUILD_PLAN/ADR pass that picks the exact track-record threshold (N consecutive unreverted merges).
...
3. **The full cast stands up together** once the foundation exists, so cross-persona dynamics (banter, disagreement, handoffs) can be tested from day one rather than inferred from a lone persona.
...
Working-hours findings, designer-split timing, tie-breaking validation, cost calibration, per-project channel depth, persona-to-persona DM volume, sick-day handling, exit criteria, interjection-bar mechanism (the threshold governing when a persona speaks up unprompted, relevant to §1.3's Marcus example — not yet resolved), Slack-status-away parsing conventions, and the deferred persona-confidence health signal all remain open — none were touched by the diagnosis of what actually failed, so none needed re-litigating here.
