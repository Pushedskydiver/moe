...
**Pre-merge checkpoint — four questions, answered against the PR, not from memory.** Steps 5 and 6 already state their rules plainly, and both were still skipped on a real PR (chunk 5.2a: the surrogate was never dispatched despite two triggers firing; chunk 3.7: it was dispatched, its findings were fixed, and they were never posted). Restating the rules is not the fix — the rules were already correct and already written. What was missing is a point at which their _execution_ is verified, the way this step already verifies R2. So, immediately before handing the PR to Alex:

1. **Did any surrogate trigger fire?** Check mechanically, don't recall: `git diff main...HEAD --stat` for the LOC count, `git diff main...HEAD --name-only` against `docs/GIT.md`'s blast-radius list (**re-read the list — it is the source of truth, and reciting it from memory is how a doc gets mis-classified in either direction**), and `git log main..HEAD --format=%s` for any `fix(docs)` commit. Any one of those, plus "does this PR's own new prose make a factual claim about the repo or an external library", triggers dispatch.
...
3. **Were its findings posted as a PR comment?** `gh pr view <n> --json comments --jq '.comments | length'`. Zero comments on a PR that ran a surrogate pass means the audit trail is missing, regardless of whether every finding was fixed. Post it late rather than not at all, and say it is late.
...
**The specific trap that keeps catching this: a clean DA round feels like a satisfied gate.** DA and `copilot-surrogate` read differently — DA reads the diff hunting defects, the surrogate reads whole files at HEAD hunting false claims — so a thorough DA→R2 chain returning clean says nothing about whether the surrogate would find anything. On chunk 3.7 the surrogate found a BLOCKING that two DA passes had both missed. "DA and R2 already covered it" is the same reasoning `docs/decisions/REVIEW-GATE-DISCRETION.md` was written to retire, and it is worth naming here because it recurs in a form that feels like diligence rather than shortcutting.
