...
| `apps/server`     | The deployable long-running process — one instance per persona | Boots, health-check endpoint, structured logging, Slack Socket Mode wiring (chunks 2.2/2.3); thread-scoped conversation history via its first `@moe/core`/DB dependency (chunk 2.4b); wires the `report_status` claim gate into the reply path (chunk 2.5); accounts each turn's LLM cost against the persona/day bucket (chunk 2.6a); checks the cost cap before every LLM call, alerting/hard-halting per persona/month (chunk 2.6b)                                                                                                                                                                    |
