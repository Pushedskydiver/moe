...
| `packages/agents` | Persona definitions (prompts, per-persona config)              | `PersonaConfig` schema + parser (chunk 2.2, extended with `slackAppToken` at 2.3); persona prompts not yet built (Stage 5, do-not-touch)                                             |
