...
This is a **living document** — add patterns from real catches, not hypotheticals, with one deliberate exception: the five patterns below are **pre-seeded per `docs/VISION.md` §12**, which names them explicitly as "the highest-confidence failure classes … so the review agent has something to consult on day one" (the ellipsis stands in for §12's own five-item parenthetical, which lists the same five patterns). Some describe functionality that doesn't exist yet (a persona doesn't exist until Stage 5) — they're seeded ahead of any real catch, not evidence of one. The core-hours guard itself now exists (`BUILD_PLAN.md` chunk 2.7a) — its own pattern entry below reflects that; as of `BUILD_PLAN.md` chunk 3.4a-i, that pattern's _predicted failure_ (a new proactive call site skipping the check) has actually fired once, on the guard's first real consumer (see its `Caught:` status below); recorded-transcript drift graduated the same way at BUILD_PLAN 5.3g, its own staleness gate confirmed working against a real post-5.4 edit to already-shipped prompts (see its own entry below) — the other three pre-seeded patterns below remain unobserved. Each pattern says so explicitly rather than pretending to a track record moe doesn't have yet. Once a pre-seeded pattern is actually triggered by a real PR, replace its "pre-seeded" marker with a real `Caught:` citation — that's the moment it graduates from anticipation to evidence.
...
**Status: Caught, BUILD_PLAN 5.3g, by a spec-grill codebase claim-extraction pass on Theo's draft.** Two real inaccuracies, both present in substance across all six previously-shipped prompts (Sarah, Maya, Marcus, Riley, Priya, Dom — Sarah's own copy of finding (2) below carried two extra words, "in the message," not present in the other five, but the same underlying false claim), not introduced by Theo's own draft: (1) "a separate, already-calibrated mechanism (`classify-message-confidence.ts`)" named the wrong file for where the High/Mid/Low **band** is actually computed — `classify-message-confidence.ts` only returns a raw numeric score; the band itself comes from `classifyConfidenceBand` in `packages/core/src/confidence-band.ts`, a different file in a different package. Fixed by dropping the specific filename entirely ("a separate, already-calibrated pipeline upstream of you") rather than naming a second specific file, so the sentence can't drift again if the pipeline is refactored further — applied identically across all seven prompts, modulo Sarah's own pre-existing "that's a separate..." lead-in versus the other six's "a separate..." (her own sentence structure from before this fix, not a new inconsistency). (2) The High-band paragraph's "Lead with the action: you're drafting this, then the specific line(s) that told you so" described narration `compose-ticket-draft.ts`'s own fixed `DRAFT_SYSTEM_PROMPT` structurally cannot produce — the actual output is a `{title, body}` pair required to restate the message plainly, with no field for meta-commentary about why a draft was made. Six prompts (Maya, Marcus, Riley, Priya, Dom, Theo) received an identical replacement sentence ("The draft itself restates the message plainly — a title and a short body, no invented cause or detail beyond what's actually there"); Sarah's own paragraph already described plain restatement in its own separate words ("a few plain sentences restating what was said, nothing more"), so her copy of the false sentence was removed outright rather than duplicating that description a second way — same fix, not identical text. Both fixes triggered a full fleet-wide replay re-recording (see Recorded-transcript drift, below, for how that gate behaved).
