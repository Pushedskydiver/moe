import type { ApprovePlanDeps } from './approve-plan-via-reaction.js';
...
function makeDeps(overrides: Partial<ApprovePlanDeps> = {}): ApprovePlanDeps {
  return {
    claimTicket: vi
      .fn<ApprovePlanDeps['claimTicket']>()
      .mockResolvedValue(makeSuccessfulClaim()),
    transitionTicket: vi
      .fn<ApprovePlanDeps['transitionTicket']>()
      .mockResolvedValue({ ok: true, ticket: makeTicket() }),
    releaseTicket: vi
      .fn<ApprovePlanDeps['releaseTicket']>()
      .mockResolvedValue(makeSuccessfulClaim()),
    logger: makeLogger(),
    ...overrides,
  };
...
describe('approvePlanAndTransitionToBuild', () => {
  it('returns claim-failed with the original ClaimError preserved, and attempts neither a transition nor a release, when the claim fails', async () => {
    ...
    const result = await approvePlanAndTransitionToBuild(deps, {
      ticketId: TICKET_ID,
      projectKey: PROJECT_KEY,
