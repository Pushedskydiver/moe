 ...
 * §5.3 settles that a DM sent directly to a named persona is unambiguous and is handled by that
 * persona's own app without Sarah in the loop — this predicate short-circuits Stage 0 for that
 * case rather than gating on it, since §5.3 itself is silent on Stage 0 specifically. So
 * `MessageSurface` only needs to distinguish "channel" (subject to the allow-list below) from
 * "dm" (never checked). Deliberately has no notion of Slack's own `channel_type`
 * (`channel`/`group`/`im`/`mpim`) — `packages/core` has zero Slack dependency; a caller
 * normalizes a real Slack event into this shape before calling `isSurfaceInScope`.
 */
