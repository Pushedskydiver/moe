...
**Status: Stage 0 in progress.** The monorepo is scaffolded and the commands below run for real — this is no longer a pre-scaffold description of a target state. `docs/ARCHITECTURE.md`, `docs/GLOSSARY.md`, `docs/decisions/` (with its README.md brief/design-doc/trim-to-decisions lifecycle convention), `docs/VISION.md`, `docs/CONVENTIONS.md`, `docs/GIT.md`, `docs/DA-REVIEW.md`, `docs/SELF-REVIEW.md`, `docs/DEVELOPMENT.md`, `docs/TESTING.md`, `docs/RATIONALIZATIONS.md`, and `docs/REVIEW-PATTERNS.md` are all real today, along with the `da-review`/`spec-grill`/`copilot-surrogate` agent definitions in <!-- literal:start -->`.claude/agents/`<!-- literal:end --> and moe's own `PROGRESS.md`/`docs/history/SESSIONS.md` session-handoff pair. `docs/INDEX.md` is still deliberately deferred past Stage 0 (it needs real PRs to route against) — the 0.6c docs above now exist as its future routing targets, same as in chief-clancy, but INDEX.md itself remains Stage 1–2 territory, per `BUILD_PLAN.md`'s own "Deliberately not scheduled" entry. The chunk-0 doc/tooling set is now fully complete. This paragraph names what exists by file rather than by chunk number, so it stays accurate as more chunks merge — check `BUILD_PLAN.md`'s checkboxes for exactly which Stage-0 chunks have landed.
...
- **Review order: architectural → DA (subagent) → self → PR. Never skip or reorder.** The `da-review`/`spec-grill`/`copilot-surrogate` agent definitions (<!-- literal:start -->`.claude/agents/`<!-- literal:end -->) and their checklists (`docs/DA-REVIEW.md`, `docs/SELF-REVIEW.md`) are live as of chunk 0.5 — dispatch DA review from a fresh context before every non-trivial PR, per their own definitions.
...
- **AGENTS.md is generated from this file — never hand-edit it.** Run `pnpm generate:agents-md` after editing this file, then `pnpm format`. A CI check fails the build if AGENTS.md is stale relative to CLAUDE.md. This closes the gap chief-clancy's own "reader discipline only, no CI gate" sync gets away with by never letting the files diverge in the first place. Marker conventions (`source-only`, `literal`): `docs/DEVELOPMENT.md` §AGENTS.md generation.
