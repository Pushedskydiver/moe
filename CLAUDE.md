...
**Status: Stage 0 in progress.** The monorepo is scaffolded and the commands below run for real — this is no longer a pre-scaffold description of a target state. Several docs referenced below (`RATIONALIZATIONS.md`, `REVIEW-PATTERNS.md` — chunk 0.6b; `ARCHITECTURE.md`, `GLOSSARY.md`, and `docs/decisions/` with its README.md brief/design-doc/trim-to-decisions lifecycle convention — chunk 0.6c) don't exist yet — chunk-0 deliverables, not aspirational claims. `docs/INDEX.md` is deliberately deferred past Stage 0 entirely (it needs real PRs to route against) and is structurally coupled to the 0.6b–0.6c docs existing first as routing targets, same as in chief-clancy. `docs/VISION.md`, `docs/CONVENTIONS.md`, `docs/GIT.md`, `docs/DA-REVIEW.md`, `docs/SELF-REVIEW.md`, `docs/DEVELOPMENT.md`, and `docs/TESTING.md` are real today, along with the `da-review`/`spec-grill`/`copilot-surrogate` agent definitions in `.claude/agents/` and moe's own `PROGRESS.md`/`docs/history/SESSIONS.md` session-handoff pair. This paragraph names what exists by file rather than by chunk number, so it stays accurate as more chunks merge — check `BUILD_PLAN.md`'s checkboxes for exactly which Stage-0 chunks have landed.
